...
for i in {1..4}
do
    python3 scripts/train_covid19_proper.py --trainable_base_layers 3 --model_id 'vgg_base' --learning_rate 1e-4 --data_dir '../data/cross_validation_balanced' --validation_fold 2 --test_fold 3 --epochs 1 --batch_size 8 --model_name "ensemble_balanced4_part${i}" --num_models 1
done

mkdir models/ensemble_balanced4

# Copy over files
i=0
all_models=$(find ./models/ -wholename "*ensemble_balanced4_part*model-0")
for model in $all_models; do mv $model/ ./models/ensemble_balanced4/model-$i; i=$(($i+1)); done
